...
| `dataKey`                   | `string | null`                                           | `null`                                 | Dot-notation property key that uniquely identifies each row. Required for row expansion, selection equality, and `                                                                                                                                           |
| `disabled`                  | `boolean`                                                 | `false`                                | When `true`, the entire component is non-interactive.                                                                                                                                                                                                        |
| `emptyMessage`              | `string | null`                                           | `null`                                 | Message shown in the empty state when no `[uiTableEmpty]` template is provided.                                                                                                                                                                              |
...
| `globalFilterPlaceholder`   | `string | null`                                           | `null`                                 | Placeholder displayed inside the global filter input.                                                                                                                                                                                                        |
...
| `rowClass`                  | `((row: unknown, index: number) => string | null) | null` | `(row: unknown`                        | Optional function that returns additional CSS class(es) to apply to each body row. Called once per rendered row with the row data object and its zero-based display index. Return a class string, space-separated classes, or `null` to apply nothing extra. |
...
| `--uilib-table-expander-border-radius`  | `var(--uilib-radius-full, 9999px)`                         |
...
| `--uilib-table-transition`              | `var(--uilib-transition-fast, 150ms ease)`                 |
