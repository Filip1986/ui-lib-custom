...
## Animation Strategy
- CSS-only expand/collapse using grid-row technique: `.ui-lib-accordion-panel__content` switches `grid-template-rows` between `0fr` and `1fr` with transition vars (`--uilib-accordion-content-animation-duration`, `--uilib-accordion-content-animation-easing`).
