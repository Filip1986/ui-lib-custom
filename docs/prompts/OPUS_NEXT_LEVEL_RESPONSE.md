...
document: most of the "missing 20%" is the _same 20%_ on every component.
...
| ------ | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| A      | Close all 8.x → 9.5+ first, _then_ build data grid                                  | ❌ Too slow; the brand needs a flagship to convert evaluators      |
...
The 11 scoring categories cover _quality dimensions_. Each component is also built from
_structural layers_. A complete DOD covers both. Layer → category mapping:
...
A component is **done** only when all 14 layers exist _and_ all 11 categories ≥ 9. The
...
```html
@if (isOpen()) { @defer (on viewport; prefetch on idle) {
...
export type UiLibSelectMode<T> =
  ...
  | {
      ...
      onChange: (next: readonly [T, T] | null) => void;
    };
```
