...
// ✅ Correct — no on* prefix; also apply a component qualifier when the name is a native DOM event
...
public readonly buttonChange: OutputEmitterRef<ValueEvent> = output<ValueEvent>();
```

> **Note:** Rule 1 and Rule 2 interact. Dropping `on*` from `onFocus` gives `focus`, which is still a blocked name under Rule 2. Always apply both rules together: no `on*` prefix **and** no native DOM event name.
...
> **camelCase variants are also blocked.** Angular normalises event names when binding — `(keyUp)` on a component resolves the same as `keyup` on a native element. An output named `keyUp` shadows `keyup`; an output named `mouseDown` shadows `mousedown`. Treat the camelCase form of any blocked name as equally blocked: `keyUp`, `keyDown`, `mouseDown`, `mouseUp`, `pointerDown`, `pointerUp`, `dragStart`, `dragEnd`, `touchStart`, `touchEnd`, etc.
...
### Canonical output names — approved library-wide
...
|---|---|---|---|
| Value changed | `change` | `{component}Change` or a semantic verb | `sliderChange`, `switchChange`, `toggleButtonChange`, `colorChange`, `checkboxChange` |
| Focus gained | `focus` | `{component}Focus` | `toggleButtonFocus`, `switchFocus`, `autocompleteFocus`, `datePickerFocus` |
| Focus lost | `blur` | `{component}Blur` | `toggleButtonBlur`, `switchBlur`, `autocompleteBlur`, `datePickerBlur` |
